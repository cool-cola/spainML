...
def data_preprocess(path1, path2, orig_reso):
	...
	i = 0
	X2 = np.zeros((good_count, orig_reso[0] * orig_reso[1])) # spainwang: X2的shape：1500*（orig_reso[0] * orig_reso[1]）
	for filename in glob.glob(path1):
		inp_image = ms.imread(filename, mode='L')
		X2[i] = inp_image.flatten() #spainwang: [x,x,x,x,x,....]
		i = i + 1
	i = 0
	...
	for j in np.arange(0, max_iter):
		...
		for batchX, batchY in get_batch(X, y_new, batch_size):
			...
			if (k % 1 == 0):
				...
				if k == 0:
					print("Epoch " + str(j + 1) + " in " + str(k + 1) + " iterations" + " Error rate :  " + str(
						error) + "%" + " loss: " + str(cost(a3, batchY, {'Theta1': theta1, 'Theta2': theta2}, lambdaa)))
				else:
					print("Epoch " + str(j + 1) + " in " + str(k + 1) + " iterations" + " Error rate :  " + str(
						error) + "%" + " loss: " + str(cost(a3, batchY, {'Theta1': theta1, 'Theta2': theta2}, lambdaa)))
...
X, X1, y, y1 = train_test_split(X, y, test_size=0.3, random_state=10) #spainwang: X_train,X_test, y_train, y_test
